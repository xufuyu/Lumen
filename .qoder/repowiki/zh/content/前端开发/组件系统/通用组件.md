...
- InstallHint
  ...
  - 关键能力：beforeinstallprompt 事件监听、安装时机判断、失败回退文案。
...
- InstallHint
  ...
  - 提示不显示：检查 beforeinstallprompt 事件是否触发。
- TooltipIcon
...
## 结论
上述六个通用组件覆盖了音频录制、数据导出、提示展示、统计展示、PWA 安装与工具提示等常见场景。通过清晰的配置项、事件回调与样式定制方法，开发者可以快速集成并满足多样化需求。建议在项目中统一封装与主题化，以提升一致性与可维护性。
