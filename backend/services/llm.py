...
- **未来意图关键词（务必纳入）**：
  只要用户说出以下关键词之一 + 一个具体动作，**必须**产出一条任务，状态默认"待办"：
  - 「我接下来要/需要/得/打算/准备/计划 XXX」
  - 「我下一步 XXX」/「接下来 XXX」/「等会儿要 XXX」
  - 「我要 XXX」/「我得 XXX」/「我该 XXX」
  - 「明天/后天/下周 要 XXX」
  即使句子里也可能被别的模块识别成"事件"（比如"我接下来需要优化前端"），
  在**任务这一路**依然要纳入，不要靠推理"是不是别的类别更合适"来省略。
- **状态判定（关键）**：
...
async def answer_query(question: str, context_json: str) -> str:
    """用用户的记录作为上下文回答自然语言问题。"""
    prompt = f"""仅使用提供的记录作为上下文来回答用户的问题。
如果记录中没有足够的信息来给出有把握的回答，请明确说明。

用户问题：{question}

可用记录（JSON 数组，每项含 id、content、created_at）：
{context_json}

返回一个 JSON 对象：
{{
  "回答": "你的回答。直接、实事求是。如果无法回答，说'我目前没有足够的信息来回答这个问题'。",
  "来源": [{{"记录ID": 1, "摘录": "相关片段 ≤100 字", "创建时间": "ISO 时间"}}],
  "免责声明": "关于不确定性的简短说明；如果回答有充分依据则为 null"
}}

重要：
- 仅根据提供的记录回答，不要超出记录范围进行推断。
- 如果多条记录相关，全部引用。
- 如果问题涉及医疗/心理健康建议，请附加免责声明，说明你是一个辅助工具，而非医疗资源。
- 友善且实事求是。用户可能患有抑郁、ADHD 或解离障碍。
