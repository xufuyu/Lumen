...
[
  {{
    ...
    "状态": "待办" | "进行中" | "已完成",
    "来源记录ID": [1],
    "更新任务ID": 123  // 可选：如果要更新已有任务的状态，填对应 ID；新任务省略此字段
  }}
]
...
- 不要编造用户没提到的任务。
- **状态更新识别（关键）**：
  仔细对比已有任务列表，判断用户输入是否是在**完成或推进**某个已有任务。
  匹配时看**语义**而非字面：
  - 已有任务「去吃晚饭」+ 用户说「我已经吃过晚饭了」→ 这是完成！
  - 已有任务「跑步锻炼」+ 用户说「刚跑完步」→ 完成！
  - 已有任务「写周报」+ 用户说「周报写了一半」→ 进行中！

  **如果要更新已有任务的状态，必须输出 `更新任务ID` 字段（从已有任务列表中找对应的 ID），而不是创建新任务：**
  ```json
  {{"更新任务ID": 123, "状态": "已完成"}}
  ```
  **如果不输出 `更新任务ID`，系统会当作新任务创建，导致重复！**
